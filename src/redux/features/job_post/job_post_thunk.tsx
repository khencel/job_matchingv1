...
import type { PostBasicInfoState, UpdateJobPostPayload } from "@/redux/slices/employer/post_a_job/basicInfoSlice";
import postAPI from "@/redux/features/api/api_request";
import { fetchAPI, standard_get_api, standard_delete_api, standard_update_api } from "@/redux/features/api/api_request";
...
export const updateJobPost = createAsyncThunk(
    ...
    async (jobData: UpdateJobPostPayload, {rejectWithValue}) => {
        try {
            ...
            const response = await standard_update_api(`/api/job/update/${jobData.id}`, payload);
            return response.data;
        } catch (error: any) {
            return rejectWithValue(error.response.data);
        }
